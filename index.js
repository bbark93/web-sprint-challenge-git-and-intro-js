...
const shortArray = removeArtist(artists,0);

console.log('task 5:', shortArray);
...
const largerArray = addArtist(artists, "john Doe", "1988-2022", "Full Stack Development", "African American", "I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.");

console.log('task 6:', largerArray);
...
function artistInfo(array, str1){
  for (let i = 0; i < array.length; i++) {
    if (array[i].name === str1) {
      return array[i].bio;
    }
  }
}
...
function artistByCountry(array, str1){
  const tempArray = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === str1) {
      tempArray.push(array[i].name);
    }
  }
  return tempArray;
